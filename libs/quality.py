...
class DeequParser:
    ...
    def parse_json_check_suite(cls, check: Check, check_suite: list) -> Check:
        ...
        for _check in check_suite:
            getattr(check, _check["check"])(*_parse_params(_check["params"]))
    ...
    def parse_output_result_to_dataframe(
        ...
        """
        return (
            VerificationResult.checkResultsAsDataFrame(spark, check_result)
    ...
    def parse_team_html_output(cls, table, batch_id, result_details):
        ...
        html_stable = jinja2_template.render(result_details=result_details)
        ...
        sections = [
            ...
            {"text": html_stable, "markdown": False},
        ]
...
class DeequQuality:
    ...
    def _parse_summary(self, _dq):
        df = (
            _dq.groupBy("target_table", "run_id", "run_dt", "run_dts")
            .pivot("constraint_status")
            .agg(F.count("*"))
        )
        ...
        df = (
            df.withColumn("success", _gen0col("Success"))
            .withColumn("failure", _gen0col("Failure"))
            .withColumn("total", F.col("failure") + F.col("success"))
            .withColumn(
                "success_percent", F.round(F.col("success") / F.col("total") * 100, 2)
            )
        )
    ...
    def validate(self, df):
        ...
        check_result = (
            VerificationSuite(self.spark).onData(df).addCheck(self.check).run()
        )

        result_df = DeequParser.parse_output_result_to_dataframe(
            check_result, self.target_table, run_id, self.spark
        )
