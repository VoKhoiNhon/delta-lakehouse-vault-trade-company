...
class SqlCountPair(ReconcilitionPair):
    def __init__(
        ...
        "target_sql": '''
        select
        (select count(*) from iceberg_catalog.`maxenv-t2_db_silver`.hub_listing where dv_recsrc = 'source_g') as cnt_hub_listing,
        (select count(*) from iceberg_catalog.`maxenv-t2_db_silver`.sat_glisting) as cnt_sat_glisting,
        (select count(*) from iceberg_catalog.`maxenv-t2_db_silver`.link_account_listing_district where dv_recsrc = 'source_g') as cnt_link_ald
        '''
        }
        """
    ...
    def compare(self, source_result: dict, target_result: dict) -> bool:
        ...
        diff = dict((k, target_result[k] - source_result[k]) for k in target_result)
        return {
            "status": self.status,
...
class ReconcilitionSuite:
    def __init__(
        self, spark, reconciler: ReconcilitionPair, suite: list, jinja_data: dict = {}
    ):
        self.suite = suite
        self.jinja_data = jinja_data
    ...
    def validate(self, output_format="json"):
        ...
        if output_format == "dataframe":
            ...
            return (
                self.spark.createDataFrame(data=suite_rs, schema=schema),
                self.spark.createDataFrame(data=[summary_rs], schema=schema_summary),
                all_status,
            )
