...
class SCompanyDemographicTurkeyExecuter(SatelliteVaultExecuter):
    def transform(self):
        source = self.input_dataframe_dict["bronze.turkey"]
        ...
        df = (
            df.withColumn("jurisdiction", F.lit("Turkey"))
            .withColumn("is_branch", F.lit(False))
            .withColumn("country_code", F.lit("TR"))
            .withColumn("country_name", F.lit("Turkey"))
        )
...
def run():
    ...
    executer_hub = SCompanyDemographicTurkeyExecuter(
        sys.argv[0],
