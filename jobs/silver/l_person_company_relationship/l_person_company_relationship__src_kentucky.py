...
class LinkPositionExecuter(LinkVaultExecuter):
    def transform(self):
        ...
        df = df.withColumn("dv_hashkey_company", F.expr(DV_HASHKEY_COMPANY)).withColumn(
            "dv_hashkey_person", F.expr(DV_HASHKEY_PERSON)
        )
        print(df.columns)

        df = df.selectExpr(
            f"{DV_HASHKEY_L_PERSON_COMPANY_POSITION} as dv_hashkey_position",
...
def run(env="pro", params={}):
    table_meta_link_person_company_relationship = TableMeta(
    ...
    executer_link_relationship = LinkPositionExecuter(
        ...
        table_meta_link_person_company_relationship.model,
        table_meta_link_person_company_relationship.input_resources,
        params,
    )
