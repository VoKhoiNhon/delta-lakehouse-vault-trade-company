...
class LinkPositionExecuter(RawVaultExecuter):
    def transform(self):
        ...
        source = self.input_dataframe_dict["1tm_2412.entity_link_compvscomp"]
        df = source.dataframe
        record_source = source.record_source

        df = (
            df.selectExpr(
                f"{DV_HASHKEY_FROM_COMPANY} as dv_hashkey_from_company",
                f"{DV_HASHKEY_TO_COMPANY} as dv_hashkey_to_company",
