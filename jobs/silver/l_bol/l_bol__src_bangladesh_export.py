...
class LbolBangladeshExportExecuter(RawVaultExecuter):
    def transform(self):
        input_dataframe_dict = self.input_dataframe_dict
        bangladesh_trade_export = input_dataframe_dict[
            "silver.bangladesh.export"
        ].dataframe
        ...
        bangladesh_trade_export.createOrReplaceTempView("bangladesh_trade_export")
        ...
        query = f"""
         select
            ...
            jurisdiction
         from (select
                coalesce(cb.dv_hashkey_company,'CANT MAP') as supplier_dv_hashkey_company,
                coalesce(ca.dv_hashkey_company,'CANT MAP') as buyer_dv_hashkey_company,
                NULL as import_port,
                NULL as export_port,
                bte.*
            from bangladesh_trade_export  bte
            left join company_bangladesh cb
                on bte.pure_seller = cb.pure_name
                and bte.seller_country = cb.jurisdiction
            left join company_all ca
                on bte.pure_buyer = ca.pure_name
                and bte.buyer_country = ca.jurisdiction) joined_df
        """
...
def run(payload=None):
    ...
    executer = LbolBangladeshExportExecuter(
        app_name="silver_l_bol_bangladesh_export",
