# Example run:
# python3 jobs/bronze/sanction/sanction_ownership.py
...
class Executer(BronzeExecuter):
    ...
    def transform(self):
        df = self.input_dataframe_dict["raw.sanction.entity_link.ownership"].dataframe
        df = self.flatten_company(df)
        df = add_load_date_columns(df=df, date_value=self.meta_table_model.load_date)
        df = clean_column_names(df)
        df.createOrReplaceTempView("sanction_ownership")

        # Spark SQL query to filter and drop columns
        query = """
            SELECT
                owner[0] AS from_entity_id,
                asset[0] AS to_entity_id,
                'Beneficial Owner' AS position,
                2 AS position_code,
                CASE WHEN REGEXP_LIKE(start_date[0], '\\d{4}-\\d{2}-\\d{2}') THEN CAST(start_date[0] AS DATE) WHEN REGEXP_LIKE(start_date[0], '\\d{4}-\\d{2}') THEN CAST(CONCAT(start_date[0], '-01') AS DATE) ELSE CAST(CONCAT(start_date[0], '-01-01') AS DATE) END AS start_date,
                CASE WHEN REGEXP_LIKE(end_date[0], '\\d{4}-\\d{2}-\\d{2}') THEN CAST(end_date[0] AS DATE) WHEN REGEXP_LIKE(end_date[0], '\\d{4}-\\d{2}') THEN CAST(CONCAT(end_date[0], '-01') AS DATE) ELSE CAST(CONCAT(end_date[0], '-01-01') AS DATE) END AS end_date,
                schema,
                load_date
            FROM
                sanction_ownership
...
def run(payload=None):
    ...
    executer = Executer(
        app_name="bronze.sanction.entity_link.ownership",
        ...
        filter_input_resources=['raw.sanction.entity_link.ownership']
    )
    executer.execute()
