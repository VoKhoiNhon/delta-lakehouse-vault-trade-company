# Example run:
# python3 jobs/bronze/sanction/sanction_company.py
...
class Executer(BronzeExecuter):
    ...
    def transform(self):
        df = self.input_dataframe_dict["raw.sanction.company.company"].dataframe
        df = self.flatten_company(df)
        df = add_load_date_columns(df=df, date_value=self.meta_table_model.load_date)
        df = clean_column_names(df)
        df.createOrReplaceTempView("sanction_company")

        # Spark SQL query to filter and drop columns
        query = """
        select sanction_company_alias.*,
            COALESCE(main_country, country, jurisdiction,'Unknown') AS country_name,
        ...
        from (SELECT
            COALESCE(jurisdiction[0],main_country[0],country[0],'Unknown') AS jurisdiction,
            ...
            CASE WHEN kpp_code IS NOT NULL THEN CONCAT_WS('-', kpp_code) ELSE NULL END AS kpp_code,
            CASE WHEN ogrn_code IS NOT NULL THEN CONCAT_WS('-', ogrn_code) ELSE NULL END AS ogrn_code,
            CASE WHEN perm_id IS NOT NULL THEN CONCAT_WS('-', perm_id) ELSE NULL END AS perm_id,
            CASE WHEN bik_code IS NOT NULL THEN CONCAT_WS('-', bik_code) ELSE NULL END AS bik_code,
            CASE WHEN cage_code IS NOT NULL THEN CONCAT_WS('-', cage_code) ELSE NULL END AS cage_code,
            ...
            CASE WHEN ric_code IS NOT NULL THEN CONCAT_WS('-', ric_code) ELSE NULL END AS ric_code,
            ...
            COALESCE(sector[0], NULL) AS sector,
            COALESCE(status[0], NULL) AS status,
            COALESCE(program[0], NULL) AS program,
            ...
            array_contains(topics, 'sanction') AS is_sanctioned,
            email AS emails,
            phone AS phone_numbers,
            website AS websites,
            main_country[0] as main_country,
            country[0] as country,
            caption as company_name,
            schema,
            load_date
        FROM sanction_company
        WHERE schema = 'Company') as sanction_company_alias
        """
...
def run(payload=None):
    ...
    executer = Executer(
        app_name="bronze.sanction.company.company",
        ...
        filter_input_resources=['raw.sanction.company.company']
    )
    executer.execute()
