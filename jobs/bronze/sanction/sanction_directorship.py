# Example run:
# python3 jobs/bronze/sanction/sanction_directorship.py
...
class Executer(BronzeExecuter):
    ...
    def transform(self):
        df = self.input_dataframe_dict["raw.sanction.entity_link.directorship"].dataframe
        df = self.flatten_company(df)
        df = add_load_date_columns(df=df, date_value=self.meta_table_model.load_date)
        df = clean_column_names(df)
        df.createOrReplaceTempView("sanction_directorship")

        # Spark SQL query to filter and drop columns
        query = """
            SELECT
                director[0] AS from_entity_id,
...
def run(payload=None):
    ...
    executer = Executer(
        app_name="bronze.sanction.entity_link.directorship",
        ...
        filter_input_resources=['raw.sanction.entity_link.directorship']
    )
    executer.execute()
